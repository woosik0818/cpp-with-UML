...
void InquiryIngVoteControl::getMenuIndex(InquiryIngVoteUI UserInterface, MainUserCollection* userCollect, MainVoteCollection* voteCollect, Timer* timer){
	...
	UserInterface.startInterface(this, userCollect, voteCollect, timer); // 투표 리스트들을 전부 보내주고 계산해서 출력하도록 함수를 부른다.
}

Vote* InquiryIngVoteControl::getVote(int voteCount){ // 투표 리스트를 반환한다.
